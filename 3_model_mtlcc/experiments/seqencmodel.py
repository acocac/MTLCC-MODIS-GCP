...
tf.app.flags.DEFINE_integer("convrnn_filters", 32, "number of convolutional filters in ConvLSTM/ConvGRU layer")
...
tf.app.flags.DEFINE_integer("pix250m", 24, "number of 250m pixels")
...
class Model():
    def __init__(self):
        ...
        self.iterator_handle = tf.compat.v1.placeholder(tf.string, shape=[],name="data_iterator_handle")
        ...
        self.is_train = tf.compat.v1.placeholder_with_default(tf.cast(True, tf.bool), shape=(), name="is_train")
    ...
    def build_graph(self, modelpath=None):
        ...
        n_trainparams = count_params(collector_fun=tf.compat.v1.trainable_variables)
        ...
        if modelpath is not None:
            ...
            tf.compat.v1.train.export_meta_graph(filename=path)
    ...
    def input(self):
        ...
        iterator = tf.compat.v1.data.Iterator.from_string_handle(self.iterator_handle,
                                                               output_types=eval(FLAGS.expected_datatypes))

        with tf.name_scope("raw"):
            x250, x500, doy, year, labels = iterator.get_next()

            self.x250 = tf.cast(x250, tf.float32, name="x250")
            self.x500 = tf.cast(x500, tf.float32, name="x500")
            self.doy = tf.cast(doy, tf.float32, name="doy")
            self.year = tf.cast(year, tf.float32,name="year")
            self.y = tf.cast(labels, tf.int32,name="y")

        #self.x250.set_shape([2,46,24,24,5])

        # integer sequence lenths per batch for dynamic_rnn masking
        self.seq = sequence_lengths = tf.reduce_sum(tf.cast(self.x250[:, :, 0, 0, 0] > 0, tf.int32), axis=1, name="sequence_lengths")
        ...
        with tf.name_scope("reshaped"):
            ...
            b = tf.shape(self.x250)[0]
            t = tf.shape(self.x250)[1]
            px = tf.shape(self.x250)[2]

            #b,t,w,h,d = self.x250.shape()

            x500 = tf.identity(resize(self.x500, px, px),name="x500")

            tf.compat.v1.add_to_collection(ADVANCED_SUMMARY_COLLECTION_NAME,tf.identity(self.x250, name="x250"))
            ...
            x = tf.concat((self.x250,x500,doymat,yearmat),axis=-1,name="x")
            ...
            self.y.set_shape((None,None,FLAGS.pix250m,FLAGS.pix250m))
    ...
    def inference(self,input):
        ...
        if FLAGS.aggr_strat == 'state':
            ...
            logits = conv_bn_relu(input=class_input, is_train=self.is_train, filter=self.num_classes, kernel=classkernel, dilation_rate=(1,1), conv_fun=tf.keras.layers.Conv2D, var_scope="class")
    ...
    def optimize(self, loss, name):
        ...
        batchsize = tf.shape(self.x250)[0]
        samples_seen_increment_op = tf.compat.v1.assign(self.samples_seen, self.samples_seen + batchsize)
        ...
        samples_seen_increment_op2 = tf.identity(samples_seen_increment_op)  # <--- bug fixed to work with >TF1.4 this based on https://stackoverflow.com/questions/57385532/adamoptimizer-defies-tf-control-dependencies
        ...
        # with tf.control_dependencies([samples_seen_increment_op]): # execute this every time global step is incremented
            optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=lr, beta1=beta1, beta2=beta2, epsilon=FLAGS.epsilon)
    ...
    def summary(self):
        ...
        tf.compat.v1.summary.scalar("oa", self.overall_accuracy)
        tf.compat.v1.summary.scalar("xe", self.loss)
        ## histograms
...
def convrnn_layer(input, filter, is_train=True, kernel=FLAGS.convrnn_kernel, sequence_lengths=None, bidirectional=True,
    ...
    with tf.compat.v1.variable_scope(scope):
        ...
        if convcell == 'gru':
            cell = utils.ConvGRUCell((px, px), filter, kernel, activation=tf.nn.tanh, normalize=FLAGS.convrnn_normalize)
            ...
            cell = utils.ConvLSTMCell((px, px), filter, kernel, activation=tf.nn.tanh, normalize=FLAGS.convrnn_normalize,peephole=FLAGS.peephole)
        else:
        ...
        if convcell=='lstm':
            ...
            tf.compat.v1.add_to_collection(ADVANCED_SUMMARY_COLLECTION_NAME, tf.identity(concat_outputs, name="outputs"))
            tf.compat.v1.add_to_collection(ADVANCED_SUMMARY_COLLECTION_NAME, tf.identity(concat_final_state, name="final_states"))
...
def count_params(collector_fun=tf.compat.v1.trainable_variables):
    size = lambda v: functools.reduce(lambda x, y: x * y, v.get_shape().as_list())
    n = sum(size(v) for v in collector_fun())
    return n

def conv_layer(input, filter, kernel, dilation_rate=(1, 1, 1), stride=1, conv_fun=tf.keras.layers.Conv3D, layer_name="conv"): #based on https://github.com/tensorflow/tensorflow/issues/26145
    ...
    with tf.name_scope(layer_name):
        ...
        network = conv_fun(use_bias=False, filters=filter, kernel_size=kernel, strides=stride,padding='VALID', dilation_rate=dilation_rate)(inputs=input)
...
def test2():
    ...
    def get_operation(name):
        return tf.compat.v1.get_default_graph().get_operation_by_name(name).outputs[0]
    ...
    with tf.Session() as sess:
        ...
        #x250, x500, x60, doy, year, labels = sess.run(iterator.get_next())

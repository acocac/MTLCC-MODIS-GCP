...
tf.app.flags.DEFINE_string('limit_batches', '', 'limit_batches')
...
def _model_fn_old(features, labels, mode, params):
  """MTLCC model
  ...
  train_op = optimize(loss_op, x, name="train_op")

  return tf.estimator.EstimatorSpec(mode, loss=loss_op, train_op=train_op)

def _model_fn_v1(features, labels, mode, params):
  """MTLCC model
  ...
  pred = tf.cast(tf.argmax(logits, axis=3), tf.int64)
  # Prediction op: Return the top k closest items for a given user or item.
  if mode == tf.estimator.ModeKeys.PREDICT:
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      logging_hook = tf.estimator.LoggingTensorHook({"accuracy": metrics_ops[0]}, every_n_iter=64)
...
def _model_fn(features, labels, mode, params):
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
      ...
      if mode == tf.estimator.ModeKeys.EVAL:
          ...
          logging_hook = tf.estimator.LoggingTensorHook({"accuracy": ao_ops[1], 'loss': loss_op,  'global_step': global_step}, every_n_iter=1)
          return tf.estimator.EstimatorSpec(mode=mode, loss=loss_op, train_op=train_op, training_hooks=[logging_hook])
      else:

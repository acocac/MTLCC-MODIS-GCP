...
calc_seq <- function(indir, tyear, scheme, dataset, lc_target){
    
    #subset dataset
    fn <- paste0(indir,"/",tyear,".tif")

    lucserie <- raster::stack(fn)

    serie_raw <- lucserie
    
    if (scheme == 'raw'){
      serie <- serie_raw
      if (endsWith(dataset, "mtlcc")) {
      ...
      if (endsWith(dataset, "mtlcc")) {
        my_rules <- c(1,1,1,2,2,2,3,3,3,4,4,4,5,5,5,6,7,6,7,8,7)
    ...
    ids_na = which(is.na(tab), arr.ind=TRUE)
    if (length(ids_na) > 0){
      drop = ids_na[,1]
      tab = tab[!seq_len(nrow(tab)) %in% drop, ]
    }
    ...
    return(list(global_classes, global_shortclasses, global_colors, clases, tab, tyear, scheme))
}
  ...
  for (tile in tiles){
    ...
    for (d in datasets){
      ...
      for (scheme in schemes){
        ...
        destfile = paste0(dir,"/",terrai_target,".tif")

        if(file.exists(destfile)){ 
          results <- calc_seq(dir, terrai_target, scheme, d, lc_target_years)  
          
          rlist::list.save(results, paste0(data_dir,'/','resultsraw_tyear_',terrai_target,'_',scheme,'.rdata'))
          ...
          cat('###############################')
          cat(paste0('no data for year: ',terrai_target))
        }
      }
    }
  }

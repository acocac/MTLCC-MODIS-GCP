...
#### libraries ####
pckg = c('snowfall', 'TraMineR','raster', 'reshape2', 'sampling', 'tidyr', 'plyr', 'rlist')
) 
    ...
    for (d in datasets){
      ...
      for (scheme in schemes){
        ...
        if(file.exists(destfile)){ 
          ...
          rlist::list.save(results, paste0(data_dir,'/','resultsraw_tyear_',terrai_target,'.rdata'))
          
          output <- split_seq(d, results, ratio, quantile, plot)
          
          rlist::list.save(output, paste0(data_dir,'/','resultsseq_tyear_',terrai_target,'.rdata'))

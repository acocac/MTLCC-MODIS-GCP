...
Ord_Logit_Auto <- function(Dataset,D_Var,Ind_Var,Dev_Val_Split_Per=25,Imputation=TRUE,Imputation_Per=5,Outlier_Treatment=TRUE){
  ...
  for (m in c(1:length(tempNames))){
    ...
    for (n in c(1:length(y[,1]))){
      ...
      inTraining <- createDataPartition(Dataset$group, p = .50, list = FALSE, times = 1)
      
      Dataset_D  <- Dataset[inTraining,]
      Dataset_V <- Dataset[-inTraining,]
      
      print(dim(Dataset_D)[1])
      print(dim(Dataset_V)[1])
      ...
      vglmFit <- vglm(Formula, family=multinomial(refLevel=1), data=Dataset)

      #Predict The Order
      Pred_vglmFit <- as.data.frame(VGAM::predict(vglmFit, Dataset, type="response"))
      Pred_vglmFit$Predicted <- levels(Dataset$group)[max.col(Pred_vglmFit)]
      #Correct Classification of Order
      Pred_vglmFit$Predicted <- factor(Pred_vglmFit$Predicted, levels=levels(Dataset$group))
      cTab   <- xtabs(~ group + Pred_vglmFit$Predicted, data=Dataset)
      #addmargins(cTab)
...
Output <- Ord_Logit_Auto(final_df,"group",explanatory_ext)
All_Models <- as.data.frame(Output[1])
All_Models < All_Models[sort(All_Models$AIC),]

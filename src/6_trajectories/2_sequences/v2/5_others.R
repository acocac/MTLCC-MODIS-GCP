...
target_reshape$LC <- as.integer(to_factor(target_reshape$LC))
target_reshape$tyear <- as.integer(to_factor(target_reshape$LC))


library(lcmm)
mod4 <-lcmm(
  LC ~ lyear, random = ~ lyear, subject = 'tyear', 
  mixture = ~ lyear, ng = 4, idiag = TRUE, data = target_reshape, 
  link = "thresholds"
)
...
etiquettes <- paste0("Y", 1:18, " (n=", round(n*10^-6,2), "M)")

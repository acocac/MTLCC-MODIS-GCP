...
cv_fit <- function(method, covars, data) {
  ...
  if (method %in% c("multinom", "nnet")) {
    ...
    model <- train(
      Class ~ ., data = data[, c("Class", covars)], importance = TRUE,
      trControl = model_control, na.action = na.omit,
      method = method, tuneGrid = meta_params[[method]][tuning_idx,, drop = FALSE]
    )
  }  else if (method == "ranger") {
    model <- train(
      Class ~ ., data = data[, c("Class", covars)], importance = 'permutation',
...
ranger_rf = cv_fit("rf", explanatory_ext, final_df)
...
plots.name = NULL
for (pattern in levels(final_df$Class)){
...
grid.arrange(g_t1, g_t2, g_t3, g_t4, g_t5, g_t6, ncol=3)
dev.off()

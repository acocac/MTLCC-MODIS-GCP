...
def parser_fn(predict_files, directory, class_path, n_patches, maxblocks, exportblocks, fn):
  ...
  with tf.Session(config=config) as sess:
      ...
      for n in range(n_patches):
        ...
        cop_new = aggregate_datasets2own.copernicus(labels[ :, :, :, 7 ])
        ...
        cop_new = aggregate_datasets2Tsendbazaretal.copernicus(labels[ :, :, :, 7 ])
        ESA_new = aggregate_datasets2Tsendbazaretal.ESA(labels[ :, :, :, 6 ])
        MODIS_new = aggregate_datasets2Tsendbazaretal.MODIS(labels[ :, :, :, 4 ])
        ...
        for t in range(0, len(x250_), 1):
            ...
            if exportblocks != "crossyear":
                ...
                reference = ['MCD12Q1v6raw_LCType1', 'MCD12Q1v6_LCType1', 'MCD12Q1v6raw_LCProp1', 'MCD12Q1v6_LCProp1',
                             'MCD12Q1v6raw_LCProp2', 'MCD12Q1v6_LCProp2', 'ESAraw', 'Copernicusraw', 'Copernicusnew_all2ofu',
                             'Copernicusnew_cebf2ofu', 'Copernicusnew_all2ofg','Copernicusraw_fraction','Copernicusnew_cf2others',
...
if __name__ == '__main__':
    ...
    if not os.path.basename(rootdir).startswith("tile_"):
        class_path = os.path.join(rootdir,'gz',str(int(psize / maxblocks)), dataset,'classes','data' + tyear[2:])
        if not os.path.exists(class_path):
            os.makedirs(class_path)

        reference = [ 'MCD12Q1v6raw_LCType1', 'MCD12Q1v6_LCType1', 'MCD12Q1v6raw_LCProp1', 'MCD12Q1v6_LCProp1',
                      'MCD12Q1v6raw_LCProp2', 'MCD12Q1v6_LCProp2', 'ESAraw', 'Copernicusraw', 'Copernicusraw',
                      'Copernicusnew_all2ofu', 'Copernicusnew_cebf2ofu', 'Copernicusnew_all2ofg', 'Copernicusfrac',

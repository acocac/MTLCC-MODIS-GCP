...
def eval(args):
    ...
    dataset = inputs.Dataset(datadir=args.datadir, verbose=True, section=args.dataset, experiment=args.experiment,reference=args.reference, step=args.step)
...
#ac                        tileid = int(os.path.basename(files[tile]).split(".")[0])
                        ...
                        threadlist.append(write_tile(label[tile], files[tile],args.storedir, geotransform, srid))

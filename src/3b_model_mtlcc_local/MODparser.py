...
class MODparser():
    ...
    def parse_example_bands_fraction(self, serialized_example):
        """
        example proto can be obtained via
        filename_queue = tf.train.string_input_producer(filenames, num_epochs=None)
        or by passing this function in dataset.map(.)
        ...
        labels = tf.reshape(tf.decode_raw(feature[0]['labels_frac/data'], tf.float32), tf.cast(feature[0]['labels_frac/shape'], tf.int32))

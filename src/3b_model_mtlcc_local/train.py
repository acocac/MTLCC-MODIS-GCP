...
def setupDatasets(args):
    ...
    datasets_dict=dict()
    for section in ['2001','2002','2003']:
    # for section in ['2015']:
        datasets_dict[section]=dict()
        dataset = Dataset(datadir=args.datadir, verbose=True, temporal_samples=args.temporal_samples, section=section, experiment=args.experiment, reference=args.reference, step=args.step)
...
def train(args, datasets):
    ...
    if args.limit_batches > 0:
        ...
        #("artificially limiting batches to {} -> number of samples {}".format(args.limit_batches, num_samples))

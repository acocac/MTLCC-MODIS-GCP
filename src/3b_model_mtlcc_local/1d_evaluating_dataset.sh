    ...
    for model in ${MODELS[@]}; do
        for split in ${SPLITS[@]}; do
            ...
            for year in ${YEARS[@]}; do
                ...
                python evaluate.py "E:/acocac/research/${project}/eval/models/$experiment/${epochs}/${model}${cell}_p${psize}pxk0px_batch${batchsize}_${trainon}_${optimizertype}_${reference}_8d_l1_bidir_${input}_fold${split}_ckp${ckp}" \
                    --datadir="F:/acoca/research/gee/dataset/${project}/gz/${psize}/multiple" \
                    --storedir="E:/acocac/research/${project}/eval/pred/$experiment/${epochs}/${model}/${model}${cell}_${trainon}_${optimizertype}_fold${split}_${reference}_${ckp}/$year" \
                    --writetiles \
                    --batchsize=1 \
                    --dataset=$year \
                    --experiment='bands' \

...
tf.app.flags.DEFINE_integer("convrnn_filters", 0, "number of convolutional filters in ConvLSTM/ConvGRU layer")
...
tf.app.flags.DEFINE_integer("pix250m", 0, "number of 250m pixels")
...
tf.app.flags.DEFINE_integer("num_classes", 8, "number of classes not counting unknown class -> e.g. 0:uk,1:a,2:b,3:c,4:d -> num_classes 4")

## performance ##
tf.app.flags.DEFINE_boolean("swap_memory", True, "Swap memory between GPU and CPU for recurrent layers")

tf.app.flags.DEFINE_string('optimizertype', None, 'optimizertype')
...
class Model():
    ...
    def build_graph(self, modelpath=None):
        ...
        self.train_op = self.optimize(self.loss, FLAGS.optimizertype, name="train_op")
    ...
    def input(self):
        ...
        with tf.name_scope("reshaped"):
            ...
            tf.add_to_collection(ADVANCED_SUMMARY_COLLECTION_NAME,tf.identity(self.x250, name="x250"))
            tf.add_to_collection(ADVANCED_SUMMARY_COLLECTION_NAME, x500)
    ...
    def optimize(self, loss, otype, name):
...
def convrnn_layer(input, filter, is_train=True, kernel=FLAGS.convrnn_kernel, sequence_lengths=None, bidirectional=True,
    ...
    with tf.variable_scope(scope):
        ...
        if FLAGS.attention == True:
            ...
            outputs = tf.stack(concat_outputs, axis=1)

            # Trainable parameters
            attention_size = FLAGS.convrnn_filters
            W_omega = tf.Variable(tf.random_normal([FLAGS.convrnn_filters, attention_size], stddev=0.1))
            b_omega = tf.Variable(tf.random_normal([attention_size], stddev=0.1))
            u_omega = tf.Variable(tf.random_normal([attention_size], stddev=0.1))

            # Applying fully connected layer with non-linear activation to each of the B*T timestamps;
            #  the shape of `v` is (B,T,D)*(D,A)=(B,T,A), where A=attention_size
            v = tf.tanh(tf.tensordot(concat_outputs, W_omega, axes=1) + b_omega)
            # For each of the timestamps its vector of size A from `v` is reduced with `u` vector
            vu = tf.tensordot(v, u_omega, axes=1)  # (B,T) shape
            alphas = tf.nn.softmax(vu)  # (B,T) shape also
            output = tf.reduce_sum(concat_outputs * tf.expand_dims(alphas, -1), 1)
            output = tf.reshape(output, [-1, px, px, FLAGS.convrnn_filters])
            print(output.shape)

            tf.add_to_collection(ADVANCED_SUMMARY_COLLECTION_NAME, tf.identity(output, name="outputs"))

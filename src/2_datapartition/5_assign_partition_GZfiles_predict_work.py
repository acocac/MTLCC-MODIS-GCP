...
if __name__ == '__main__':
    ...
    pred = parfiles(tileiddir)

    #load merge patch and fileid geojson
    df_all = gpd.read_file(os.path.join(indir, 'raw', 'geojson','merge_all.geojson'))
    #
    # #project dir
    projectdir = os.path.join(rootdir, 'combine', str(int(psize / blocks)))
    #
    # #GZ dir
    GZdir = os.path.join(projectdir, 'data' + tyear[2:])
    #

    target = ['AMZ_p384k0-{}'.format(i)for i in df_all.file_id.unique().tolist()]
    target.sort(key=lambda f: int(re.sub('\D', '', f)))

    outdir = os.path.join(projectdir, 'tileids')
    if not os.path.exists(outdir):
        os.makedirs(outdir)

    np.savetxt(os.path.join(outdir,'pred.tileids'), np.array(target).astype(str),fmt='%s')

    # create geotransform
    if not os.path.isfile(os.path.join(projectdir, 'geotransforms.csv')):

        id_n, x, y = bbox_geojson(os.path.join(indir, 'raw', 'geojson','merge_all.geojson'))

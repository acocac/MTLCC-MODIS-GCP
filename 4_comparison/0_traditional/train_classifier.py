...
#-----------------------------------------------------------------------		
def main(classifier_type, train_file, test_file, model_dir, channels):
	...
	# Output filenames
	res_file = os.path.join(model_dir,'resultOA-' + classifier_type + '.csv')
	res_mat = np.zeros((len(eval_label),1))
	model_file = os.path.join(model_dir,'model-' + classifier_type + '.h5')
	conf_file = os.path.join(model_dir,'confMatrix-' + classifier_type + '.csv')
	acc_loss_file = os.path.join(model_dir,'trainingHistory-'+ classifier_type + '.csv') #-- only for deep learning models
	...
	if dl_flag:			#-- deep learning approaches
		...
		# min_per = np.repeat(-100, 7)
		# max_per = np.repeat(16000, 7)
	...
	for add in range(nclasses):
		class_label.append("cl"+str(add+1))
...
if __name__ == "__main__":
	try:
		if len(sys.argv) == 1:
			...
			main(args.classifier, args.train_file, args.test_file, args.modeldir, args.channels)
			print("0")
	except(RuntimeError):
		print >> sys.stderr
		sys.exit(1)
